...
- REAPER の実起動が前提になる。`compose` / `check` / `render`(sim) はオフラインで
  ...
  分岐の中だけに閉じる。
- **「無人」は達成するが「ヘッドレス」にはならない。** この2つは別物で、本決定が
  手に入れたのは前者だけ。
  - 無人（人間の操作が要らない）: 達成。ダイアログが出ても番犬が閉じ、失敗を返して
    次のレンダへ進める。人間のクリックは一切不要。
  - ヘッドレス（ディスプレイが要らない）: **不可**。`DISPLAY` と `WAYLAND_DISPLAY`
    を両方 unset すると REAPER はハングする（実測、120秒タイムアウト）。**xvfb を
    入れても解決しない**（導入して確認済み）。さらに層2の復旧手段が niri の
    `close-window` である以上、コンポジタの無い環境では番犬そのものが機能しない。
  したがって **CI / サーバ上の夜間バッチではミックス検証を回せない**。運用は
  「このマシンのデスクトップで REAPER を起動しっぱなしにし、reapy でコマンドを投げる」
  形に限定される（`kita reaper start` が既にある）。これは Path B なら払わずに済んだ
  コストで、本決定の最大のトレードオフ。
